            ...
            {statuses.map(status => (
                <div key={status} className="flex-1 m-4 p-4 bg-white rounded-2xl shadow-lg min-w-[250px] max-w-[300px]">
                    ...
                    {tasks.filter(task => task.status === status).map(task => (
                        <div key={task.id} className="mb-4 p-4 bg-gray-200 rounded-lg">
                            ...
                            <button className="mt-2 bg-blue-600 text-white w-full py-2 rounded" onClick={() => moveTask(task.id, statuses[(statuses.indexOf(task.status) + 1) % statuses.length])}>
                                Move to Next Stage
                            </button>
                        </div>
                    ))}
